...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["image/jpeg", "image/jpg", "image/png", "application/pdf"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for floor-plans" ON storage.objects FOR SELECT USING (bucket_id = 'floor-plans');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for floor-plans" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'floor-plans');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for floor-plans" ON storage.objects FOR UPDATE USING (bucket_id = 'floor-plans');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for floor-plans" ON storage.objects FOR DELETE USING (bucket_id = 'floor-plans');`
    ];
